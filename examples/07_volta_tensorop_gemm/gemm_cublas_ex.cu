...
void init_vector(int* d_data, int len) {
    // a rough piece of code considering no performance 
    int* h_data = (int*) malloc(sizeof(int) * len);
    for (size_t i = 0; i < len; ++ i) {
        h_data[i] = (int) (i + 1);
    }
...
void init_vector(char* d_data, int len) {
    float* f_data = (float*) malloc(sizeof(float) * len);
    for (size_t i = 0; i < len; ++ i) {
        f_data[i] = (float)(i + 1);
    }
    char* h_data = (char*) malloc(sizeof(char) * len);
    for (size_t i = 0; i < len; ++ i) {
...
void print_C(const int* d_mat, int m, int n) {
    // a rough piece of code considering no performance 
    int* h_mat = (int*) malloc(sizeof(int) * m * n);
